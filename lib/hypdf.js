...
HyPDF.prototype.request = function (endpoint, data, _callback) {
  ...
  if(endpoint === 'htmltopdf') return request.post(this.host + endpoint, { json: json }, _callback ? callback : undefined);
  ...
  request.post({url:this.host + endpoint, formData: json}, _callback ? callback : undefined);
  ...
  function callback(err, res, body) {
    ...
    if(json.bucket) {
      ...
      console.log('here', body.url)
      response.pdf = body.url;
    } else {
      // the body is the binary pdf
      console.log('or here', body)
      response.pdf = body;
    }
...
HyPDF.prototype.pdfextract = function(file, options, callback) {
  ...
  // utils.getBuffer(file, function (err, binary_pdf_data) {
    ...
    var data = utils.merge({}, options || {}, {file: fs.createReadStream(file)});
